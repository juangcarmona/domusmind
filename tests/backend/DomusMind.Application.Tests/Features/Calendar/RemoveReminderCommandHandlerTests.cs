...
public sealed class RemoveReminderCommandHandlerTests
{
    ...
    private static RemoveReminderCommandHandler BuildHandler(
    ...
    public async Task Handle_WithValidInput_ReturnsResponse()
    {
        ...
        var result = await handler.Handle(
            new RemoveReminderCommand(evt.Id.Value, 45, Guid.NewGuid()),
            CancellationToken.None);

        result.CalendarEventId.Should().Be(evt.Id.Value);
        result.MinutesBefore.Should().Be(45);
    }
    ...
    public async Task Handle_EventNotFound_ThrowsCalendarException()
    {
        ...
        var act = () => handler.Handle(
            new RemoveReminderCommand(Guid.NewGuid(), 30, Guid.NewGuid()),
            CancellationToken.None);
    ...
    public async Task Handle_AccessDenied_ThrowsCalendarException()
    {
        ...
        var act = () => handler.Handle(
            new RemoveReminderCommand(evt.Id.Value, 30, Guid.NewGuid()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_OffsetNotFound_ThrowsCalendarException()
    {
        var (db, evt) = await BuildWithEventAsync();
        var handler = BuildHandler(db);

        var act = () => handler.Handle(
            new RemoveReminderCommand(evt.Id.Value, 120, Guid.NewGuid()),
            CancellationToken.None);

        await act.Should().ThrowAsync<CalendarException>()
            .Where(e => e.Code == CalendarErrorCode.ReminderOffsetNotFound);
    }
}
