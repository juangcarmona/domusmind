...
public sealed class RegenerateTemporaryPasswordCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_ReturnsNewTemporaryPassword()
    {
        var (db, family, managerUserId, _, targetMemberId) = await BuildFamilyWithLinkedMemberAsync();
        var passwordGen = new StubTemporaryPasswordGenerator("NewTemp99");
        var handler = BuildHandler(db, passwordGen: passwordGen);

        var result = await handler.Handle(
            new RegenerateTemporaryPasswordCommand(family.Id.Value, targetMemberId, managerUserId),
            CancellationToken.None);

        result.TemporaryPassword.Should().Be("NewTemp99");
        result.MustChangePassword.Should().BeTrue();
    }

    [Fact]
    public async Task Handle_UpdatesPasswordHash()
    {
        ...
        [
            new AuthUserRecord(linkedUserId, "bob@home.local", "HASHED:OldPass", false),
        ]);
        var passwordGen = new StubTemporaryPasswordGenerator("NewPass");
        var handler = BuildHandler(db, users: userRepo, passwordGen: passwordGen);
        ...
        user.PasswordHash.Should().Be("HASHED:NewPass");
    ...
    {
        var (db, family, managerUserId, linkedUserId, targetMemberId) = await BuildFamilyWithLinkedMemberAsync();
        var refreshTracking = new TrackingRefreshTokenService();
        var handler = BuildHandler(db, refreshTokens: refreshTracking);

        await handler.Handle(
            new RegenerateTemporaryPasswordCommand(family.Id.Value, targetMemberId, managerUserId),
            CancellationToken.None);

        refreshTracking.RevokedUserId.Should().Be(linkedUserId);
    }
    ...
    public async Task Handle_NonManager_ThrowsAccessDenied()
    {
        ...
        var act = () => handler.Handle(
            new RegenerateTemporaryPasswordCommand(family.Id.Value, targetMemberId, nonManagerUserId),
            CancellationToken.None);
    ...
    public async Task Handle_MemberWithoutAccount_ThrowsInvalidInput()
    {
        ...
        family.AddMember(noAccountMemberId, MemberName.Create("Carol"), MemberRole.Adult,
        ...
        var act = () => handler.Handle(
            new RegenerateTemporaryPasswordCommand(family.Id.Value, noAccountMemberId.Value, managerUserId),
            CancellationToken.None);
