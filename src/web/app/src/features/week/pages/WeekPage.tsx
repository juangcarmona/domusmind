...
export function WeekPage() {
  ...
  const { grid, loading, error, weekStart, prevWeek, nextWeek } = useWeeklyGrid(familyId);
  ...
  return (
    <div className="page-content">
      ...
      {loading && <p className="loading-wrap">{t("common.loading")}</p>}
      {error && <p className="error-msg">{error}</p>}
      {!loading && !error && grid && <WeeklyGrid grid={grid} />}
      {!loading && !error && grid && grid.members.length === 0 && (
        <p className="empty-note">{t("week.empty")}</p>
      )}
    </div>
  );
}
